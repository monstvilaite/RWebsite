...
immunity (protection) from future infections. Usually a person's first infection with rotavirus causes the most severe symptoms \cite{8the}.
...
In the US, both RotaTeq and Rotarix are already licensed for routine use to prevent severe RVGE in infants and children. In Europe, only a few countries have introduced national vaccination programs, among them Germany, Finland, Belgium and Luxemburg \cite{vac}. 
...
* **age structure** and differing **susceptibility** and \textbf{contact structure} between age groups.
* Since reinfection for vaccinated as well as not vaccinated individuals is possible, the model also has to account for  \textbf{natural and vaccination induced waning of immunity}. 
* We aim to model transmission of rotavirus accounting for \textbf{severity of the disease} by distinguishing between mild and severe cases.
* Since Rotavirus vaccine does not lead to full immunity (leaky vaccine) when received, \textbf{vaccine efficacy} has to be  taken into account. 
*  Rotavirus is a seasonal disease  which peaks in September, some \textbf{seasonal force} has to be included into the model.
* Differing \textbf{recovery rates} for severe and mild cases respectively have to be modeled.
* A reduction of \textbf{infectiousness} for a mild compared to a severe infectious individual has to be taken into account.
...
+ \textbf{0.64 \% for mild cases, 100\% for severe cases }
